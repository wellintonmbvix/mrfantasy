...
uses uPrincipal, uVendas, uPesquisaWalletsDigital;

procedure TfPagtoVendas.FormClose(Sender: TObject; var Action: TCloseAction);
begin
  Action := caFree;
  fPagtoVendas := Nil;
end;

procedure TfPagtoVendas.FormCreate(Sender: TObject);
begin
   Controle := TControle.Create;
end;

procedure TfPagtoVendas.FormDestroy(Sender: TObject);
begin
  FreeAndNil(Controle);
end;

procedure TfPagtoVendas.FormKeyPress(Sender: TObject; var Key: Char);
begin
...
procedure TfPagtoVendas.Image4Click(Sender: TObject);
var
  fdqComando: TFDQuery;
begin
  ...
  Try
    ...
    Begin
      ...
      Begin
        ...
        if fVendas.jvcedt_idclie.Text <> EmptyStr then
          ParamByName('cliente').Value  := fVendas.jvcedt_idclie.Text
        else
          ParamByName('cliente').Value  := 0;
    ...
    // Deixei os checkbox visible = false p/ usuário não ter que escolher
    // no lugar trato o checked no onchanged de cada elemento
    ...
    End
      ...
      Not check_transfdep.Checked then
    Begin
      fVendas.condpagto := 'Cartão Débito';
      fVendas.parcelas := 1;
    End
      ...
      Not check_transfdep.Checked then
    Begin
      fVendas.condpagto := 'Cartão Crédito';
      fVendas.parcelas := jvsedt_parcelas.AsInteger;
    End
    else if Not check_cash.Checked AND Not check_debito.Checked AND
      Not check_credito.Checked AND check_wallet.Checked AND
      Not check_transfdep.Checked then
    Begin
      fVendas.condpagto := wallet;
      fVendas.parcelas := 0;
    End
    else if Not check_cash.Checked AND Not check_debito.Checked AND
      Not check_credito.Checked AND Not check_wallet.Checked AND
    ...
    Begin
      fVendas.condpagto := 'Transf./Dep.';
      fVendas.parcelas := 0;
    End
    else
    Begin
      fVendas.condpagto := 'Diversos';
      fVendas.parcelas := jvsedt_parcelas.AsInteger;
      // Estou passando o campo parcelas
      // porque ele pode atender aos dois tipo de cartões
    End;
...
procedure TfPagtoVendas.Image4MouseEnter(Sender: TObject);
begin
...
procedure TfPagtoVendas.Image4MouseLeave(Sender: TObject);
begin
...
procedure TfPagtoVendas.Image5Click(Sender: TObject);
begin
  Self.Close;
end;

procedure TfPagtoVendas.Image5MouseEnter(Sender: TObject);
begin
  (Sender as TImage).Height := 76;
  (Sender as TImage).Width := 76;
  (Sender as TImage).Left := (Sender as TImage).Left + 2;
...
procedure TfPagtoVendas.Image5MouseLeave(Sender: TObject);
begin
  (Sender as TImage).Height := 78;
  (Sender as TImage).Width := 78;
  (Sender as TImage).Left := (Sender as TImage).Left - 2;
...
procedure TfPagtoVendas.jvce_idcarteiraButtonClick(Sender: TObject);
begin
  nWallet := 0;
...
procedure TfPagtoVendas.jvce_idcarteiraChange(Sender: TObject);
begin
  jvv_walletdigital.Enabled := Boolean(Length(jvce_idcarteira.Text));
end;

procedure TfPagtoVendas.jvv_cartaocredChange(Sender: TObject);
var
  valor: String;
begin
...
procedure TfPagtoVendas.jvv_cartaodebChange(Sender: TObject);
var
  valor: String;
begin
  valor := SomenteNumero('R$ .,', TJvValidateEdit(Sender).Text);
...
procedure TfPagtoVendas.jvv_dinheiroChange(Sender: TObject);
var
  valor: String;
begin
  valor := SomenteNumero('R$ .,', TJvValidateEdit(Sender).Text);
...
procedure TfPagtoVendas.jvv_dinheiroEnter(Sender: TObject);
begin
  TJvValidateEdit(Sender).SelStart := Length(TJvValidateEdit(Sender).Text);
end;

procedure TfPagtoVendas.jvv_transfdepChange(Sender: TObject);
var
  valor: String;
begin
  valor := SomenteNumero('R$ .,', TJvValidateEdit(Sender).Text);
...
procedure TfPagtoVendas.jvv_walletdigitalChange(Sender: TObject);
var
  valor: String;
begin
  valor := SomenteNumero('R$ .,', TJvValidateEdit(Sender).Text);
